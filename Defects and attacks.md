...
1 | Unused State Variable (SWC-131) | Unused variables are allowed in Solidity and they do not pose a direct security issue. It is best practice though to avoid them as they can cause an increase in computations (and unnecessary gas consumption). 
 ...
 14   | Nested Call (SWC-128)                    | Nested call in a contract may cause problems such as reaching the gas limit, throwing exceptions and being inaccessible, preventing the contract from executing according to normal transaction logic or making the contract itself logically inexecutable. 
 ...
 34   | Typographical Error (SWC-129)                  | A typographical error can occur for example when a defined operation intends to sum a number to a variable (+=) but it has accidentally been defined in a wrong way (=+), introducing a typo that happens to be a valid operator. Instead of calculating the sum, it initializes the variable again. 
